...
try {
  ...
  assert.ok(installedPackage.dependencies?.canonicalize, "canonicalize must be a runtime dependency");
  assert.ok(installedPackage.dependencies?.ethers, "ethers must be a runtime dependency");
  ...
  assert.equal(typeof api.hashRuntimePayload, "function");
  assert.equal(typeof api.hashRuntimeTransition, "function");
